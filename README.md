# competitive-programming

AtCoder × C++ のコーディングテスト対策用リポジトリ。

## ディレクトリ構成

```
competitive-programming/
├── templates/
│   └── main.cpp          # 新規問題用のC++テンプレート
├── scripts/
│   ├── new.sh            # 新規問題フォルダ作成
│   ├── test.sh           # サンプルケースでの自動テスト
│   └── log_add.sh        # 振り返りログの記録
├── lib/                  # 自分用ライブラリ(コピペ用スニペット)
│   ├── union_find.hpp
│   ├── segment_tree.hpp
│   └── dijkstra.hpp
├── problems/
│   ├── _example/a/       # 動作確認用のサンプル問題(A+B問題)
│   └── <contest_id>/<problem_letter>/
│       ├── main.cpp
│       ├── sample-1.txt
│       ├── sample-1-expected.txt
│       ├── reference.md      # (任意) 解けなかった問題の解説材料
│       └── explanation.md    # (任意) AIがまとめた解法のポイント
└── log/
    └── log.csv           # 振り返りログ
```

問題は `problems/<contest_id>/<problem_letter>/` というルールで配置する。
例: ABC300のA問題 → `problems/abc300/a/`

## セットアップ

```bash
# C++コンパイラ (通常は最初から入っている)
g++ --version
```

サンプル入出力はAtCoderの問題ページから手動でコピペして配置する運用にしている

### macOSの場合の注意
macOSの `g++` は実体が Apple Clang のラッパーで、GNU専用の `bits/stdc++.h` が使えない。
AtCoderの採点環境は本物のGCCなので、Homebrewで本物のGCCを入れて使うのがおすすめ。

```bash
brew install gcc
ls /opt/homebrew/bin/g++-*   # 例: g++-16 のようにバージョン番号付きで入る
```

`test.sh` は `CXX` 環境変数で使うコンパイラを切り替えられるので、シェルの設定ファイル
(`~/.zshrc` など)に以下を追記しておくと毎回指定せずに済む。

```bash
export CXX=g++-16   # 実際にインストールされたバージョンに合わせる
```

## 使い方

### 1. 新しい問題を始める

```bash
./scripts/new.sh abc300 a
```

`problems/abc300/a/main.cpp` が作成され、次にやることが表示される。

### 2. サンプルケースを手動で配置する

表示されたURL(例: `https://atcoder.jp/contests/abc300/tasks/abc300_a`)を開き、

- 入力例1 → `problems/abc300/a/sample-1.txt`
- 出力例1 → `problems/abc300/a/sample-1-expected.txt`

にそれぞれ貼り付ける。入力例・出力例が複数ある場合は `sample-2.txt` / `sample-2-expected.txt` ...
と番号を増やしていく。

### 3. サンプルケースでテストする

```bash
./scripts/test.sh abc300 a
```

`main.cpp` をコンパイルし、`sample-*.txt` を全部入力として実行、
対応する `sample-*-expected.txt` と出力を比較して PASS/FAIL を表示する。

動作確認用に `_example/a` というサンプル問題(A+B問題)を用意しているので、
まずはこれで試すとよい:

```bash
./scripts/test.sh _example a
```

### 4. 解いたら振り返りを記録する

```bash
./scripts/log_add.sh
```

対話形式で以下を入力すると `log/log.csv` に1行追加される。

| 項目 | 説明 |
|---|---|
| date | 自動で今日の日付が入る |
| contest / problem | コンテストID・問題 |
| result | AC / WA / TLE / RE / CE / giveup など |
| time_minutes | かかった時間(分) |
| tags | 使ったアルゴリズム・分野。下記「タグ語彙」から選ぶ(複数可・カンマ区切り) |
| review | 後で復習すべきか (y/n) |
| notes | 自由記述メモ(詰まった点、学んだことなど) |
| explanation_path | 解説ファイルへのパス (例: `problems/abc447/b/explanation.md`)。未作成なら空欄 |

ログが溜まってきたら、Excelやpandasなどで `tags` 別の正答率や `review=y` の問題を
集計すると、自分の苦手分野が見えてくる。この集計を機能させるため、以下2点を運用ルールとしている。

- **tagsは自由記述ではなく、下記の統制語彙から選ぶ**。`log_add.sh` はタグ入力時に語彙表を表示し、
  語彙にない単語が入力されると警告を出す(入力は止めない)。初回と再挑戦で表記揺れ
  (例: 「連想配列」と`map`)があると同じ弱点として集計できなくなるので、必ず語彙表に揃えること。
- **同じ問題(`contest`+`problem`)を解き直したときは、上書きせず新しい行を追記する**。
  `log_add.sh` は入力されたcontest/problemの過去ログを自動検出して表示し、「再挑戦か」を尋ねる。
  再挑戦と答えると、notesの先頭に `[retry: 前回の日付 前回の結果]` が自動で付与される。
  1行=1回の挑戦の粒度を保つことで、「giveup→ACに変わった」「所要時間が縮んだ」といった
  成長の記録がそのまま残る。

### 5. 解けなかった問題の解説をまとめる (任意)

`giveup` や `review=y` にした問題は、後から解説の要点を残しておくと復習しやすい。
AIに解かせて説明させると間違った解法を正解として覚えてしまう恐れがあるため、
**公式解説や自分が解き直してACしたコードを渡した上で、AIには要点整理だけを
やってもらう** 運用にしている。

```bash
./scripts/explain.sh abc447 b
```

`problems/abc447/b/reference.md` が作成されるので、

1. 公式解説のURL、または解説の要点をコピペする
2. (解き直した場合) ACしたコードを貼る

を行ったうえで、Claude Codeなどに

```
abc447 b の解説をまとめて
```

のように依頼する。`reference.md` と `main.cpp`、`log.csv` の `notes` をもとに
`problems/abc447/b/explanation.md` に解法のポイントがまとめられるので、最後に
`log/log.csv` の該当行の `explanation_path` 列に `problems/abc447/b/explanation.md`
を記入しておく。

### 6. ライブラリを使う

`lib/` 以下のヘッダーは `main.cpp` から直接 include して使える。

```cpp
#include <bits/stdc++.h>
#include "union_find.hpp"
using namespace std;

int main() {
    UnionFind uf(10);
    uf.unite(1, 2);
    // ...
}
```

コンパイル時に `lib/` をインクルードパスに追加しているので、`test.sh` 経由で実行すれば
そのまま動く(`-I lib` を付けてコンパイルしている)。手元で直接 `g++` する場合は
`g++ -I lib problems/abc300/a/main.cpp` のように指定する。

## タグ語彙(log.csv の tags に使う標準語彙)
 
集計しやすいように、tags は以下の語彙から選んで付ける(複数可)。
ABC-C問題帯の頻出ジャンルに対応している。
 
| タグ | ジャンル | 優先度 |
|---|---|---|
| `zentansaku` | 全探索(素直な全列挙・bit全探索・順列全探索) | ★★★ |
| `map` | 連想配列・数え上げ(頻度/種類数/存在判定/ペアの個数) | ★★★ |
| `string` | 文字列操作 | ★★★ |
| `sim` | シミュレーション・実装問題 | ★★★ |
| `ruisekiwa` | 累積和・いもす法 | ★★★ |
| `nibutan` | 二分探索 | ★★★ |
| `bfs-dfs` | グリッド・グラフ探索(BFS/DFS/連結成分) | ★★☆ |
| `greedy` | 貪欲法 | ★★☆ |
| `math` | 数学・整数(約数/GCD/素数/mod/桁) | ★★☆ |
| `sort` | ソートの工夫・座標圧縮 | ★★☆ |
| `dp` | 動的計画法 | ★☆☆ |
| `shakutori` | 尺取り法 | ★☆☆ |
| `kisokusei` | 周期性・規則性の発見 | ★☆☆ |
| `adhoc` | 場合分け・コーナーケース処理 | ★☆☆ |
 
★★★を上から順に固めるのが現在の学習方針。

## 運用の流れ(おすすめ)

1. `./scripts/new.sh <contest> <letter>` で問題フォルダを作る
2. AtCoderの問題ページからサンプル入出力を `sample-1.txt` / `sample-1-expected.txt` にコピペする
3. `main.cpp` に解法を書く
4. `./scripts/test.sh <contest> <letter>` でサンプルが通るか確認
5. 通ったら(または諦めたら) `./scripts/log_add.sh` で振り返りを記録
6. 諦めた問題は `./scripts/explain.sh <contest> <letter>` で `reference.md` を作り、
   公式解説やACコードを貼ってからAIに要点をまとめてもらう
7. 週末などにまとめて `log/log.csv` を見返し、`review=y` の問題やタグ別の傾向を確認する
